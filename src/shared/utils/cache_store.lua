...
function CacheStore:new(config)
    ...
    local db_path = DataStorage:getDataDir()
        .. '/cache/'
        .. (config.db_name or 'content_cache.sqlite')
...
function CacheStore:set(key, opts)
    ...
    if success and err then
        logger.dbg('[CacheStore] Set cache key:', key, 'TTL:', ttl)
        return true
    else
        ...
        logger.dbg('[CacheStore] Failed to cache key:', key, '- likely too large for SQLite')
...
function CacheStore:clear()
    logger.info('[CacheStore] Clearing all cache entries')
    self.cache:clear()
end
