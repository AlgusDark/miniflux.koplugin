...
function BatchDownloadEntriesWorkflow.execute(deps)
    ...
    Trapper:wrap(function()
        ...
        for entry_index, entry_data in ipairs(entry_data_list) do
            ...
            local progress_message = total_entries == 1
                and T(_("Downloading: %1"), entry_title)
            ...
            if time.to_ms(time.since(time_prev)) > 1000 then
                ...
                if not user_wants_to_continue then
                    local user_choice = EntryEntity.showBatchCancellationDialog("during_batch", batch_state)

                    if user_choice == "cancel_all_entries" then
                        local cancelled_message = T(_("Batch download cancelled. Downloaded %1/%2 entries."),
                            completed_count, total_entries)
        ...
        if failed_count == 0 then
            summary_message = total_entries == 1
                and _("Download completed successfully!")
                or T(_("All %1 entries downloaded successfully!"), total_entries)
        elseif completed_count == 0 then
            summary_message = total_entries == 1
                and _("Download failed.")
                or T(_("All %1 entries failed to download."), total_entries)
        else
            summary_message = T(_("Batch download completed: %1 successful, %2 failed."),
        ...
        for img_idx, img in ipairs(images) do
            ...
            if time.to_ms(time.since(time_prev)) > 1000 then
                ...
                local image_progress = total_entries == 1
                    and T(_("Downloading:\n%1\n\nDownloading %2/%3 images"), context.title, img_idx, total_images)
                    or T(_("Downloading %1/%2:\n%3\n\nDownloading %4/%5 images"),
                        entry_index, total_entries, context.title, img_idx, total_images)

                local user_wants_to_continue = Trapper:info(image_progress)
                if not user_wants_to_continue then
                    -- User cancelled during image download - show enhanced dialog
                    local user_choice = EntryEntity.showBatchCancellationDialog("during_entry_images", batch_state)
